...
import { WorkoutLogger, type LoggerExercise, type LastPerformance } from "../../../[sessionId]/_components/workout-logger";
...
const EditSessionPage = async ({
  ...
  const { data: session } = await supabase
    .from("sessions")
    .select("id, name, started_at, ended_at, status, routine_id")
    .eq("id", sessionId)
    .maybeSingle();

  if (!session) notFound();
  if (session.status === "active") redirect(`/entrenar/${sessionId}`);
  if (session.status === "discarded") notFound();
  ...
  const [seRes, setsRes] = await Promise.all([
    ...
    supabase
      .from("sets")
      .select(
        "id, session_exercise_id, set_number, reps, weight_kg, duration_seconds, distance_meters, pace_seconds, rpe",
      )
  ...
  const [targetRes, lastRes] = await Promise.all([
    session.routine_id
      ? supabase
          ...
          .select(
            "exercise_id, target_sets, target_reps, target_weight_kg, target_duration_seconds",
          )
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 md:py-10">
      <div className="flex items-center justify-between gap-3">
        <Link
          href={`/entrenar/historial/${sessionId}`}
...
const fetchLastPerformance = async (
  ...
  for (const exerciseId of exerciseIds) {
    ...
    const { data: sets } = await supabase
      .from("sets")
      .select(
        "set_number, reps, weight_kg, duration_seconds, distance_meters, pace_seconds, is_pr",
      )
